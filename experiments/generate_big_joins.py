...
def join_query(tables):
    ...
    where_clause = ['t%d.id = t%d.id' % (t1, t2)
                    for t1, t2 in zip(tids, tids[1:])]
    where_clause = ' and '.join(where_clause)
    select_clause = ['t%d.%s' % (id, missing_col[table])
                     for id, table in zip(tids, tables)]
    select_clause = ', '.join(select_clause)
    result = 'select {} from {} where {}'.format(select_clause, from_clause,
        where_clause)
    return result
...
def create_join_workload(n_tables, n_queries):
    ...
    for _ in range(n_queries):
        random_tables = [tables[random.randint(0, len(tables) - 1)]
                         for _ in range(0, n_tables)]
